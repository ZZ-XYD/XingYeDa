...
public class OkHttpProvider {
    ...
    private static OkHttpClient getNoCacheOkHttpClient(Interceptor
                                                               cacheControl) {
        //定制OkHttp
        ...
        //设置缓存
        File httpCacheDirectory = new File(CoreApplication.getInstance().getCacheDir(), "OkHttpCache");
        ...
        httpClientBuilder.addInterceptor(new UserAgentInterceptor("Android " +
                "Device"));
//        httpClientBuilder.addInterceptor(new UrlInterceptor());
    ...
    private static class UserAgentInterceptor implements Interceptor {
        ...
        UserAgentInterceptor(String userAgentHeaderValue) {
            this.userAgentHeaderValue = Preconditions.checkNotNull
                    (userAgentHeaderValue, "userAgentHeaderValue = null");
        }
    ...
    private static class LoggingInterceptor implements Interceptor {
        ...
        public Response intercept(Chain chain) throws IOException {
            ...
            Logger.i(String.format(Locale.CHINA, "Received response for %s in" +
                            " %.1fms%n%s",
                    response.request().url(), (t2 - t1) / 1e6d, response
                            .headers()));
...
//            String host = newUrl.host();
//            if (TextUtils.isEmpty(host) || "www.example.com".equalsIgnoreCase
//                    (host)) {
//                throw new DnsXmlException();
//            }
