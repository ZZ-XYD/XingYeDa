...
//										mXiaoqu_List.add(bean);
    ...
    public static List<AdvertisementBean> ad(final Context context) {
        final List<AdvertisementBean> mList = new ArrayList<>();
                ...
                try {
                    ...
                    if (jsonArray != null && jsonArray.length() != 0) {
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ...
                            bean.setmImagePath(jsonObject.has("url") ? jsonObject.getString("url") : "");
                            bean.setmAdPath(jsonObject.has("href") ? jsonObject.getString("href") : "");
                            bean.setmTitle(jsonObject.has("title") ? jsonObject.getString("title") : "");
                            if (bean.getmImagePath().startsWith("http")) {
                                mList.add(adImage(context, bean));
                            }
                        }
                    }
                } catch (JSONException e) {
